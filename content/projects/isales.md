...
slug: isales
date: 2026-07-10
summary: "I spent five weeks trying four different ways to give a small on-device TTS model an emotion knob — decoder conditioning, cross-encoder style transfer, retrieval over a voice library, and teacher distillation. Every route to model-native emotion failed for a reason I could name, and the thing that actually shipped reads emotion instead of producing it."
tags:
  - tts
  - speech
  - pytorch
...
I wanted an emotion knob. Not acting, not a performance — a switchable style mode where a caller passes `emotion=happy` at inference and gets audibly different speech out of the same small model. The application driving it was a sales-practice iOS app where an AI customer talks back to you, and a customer who says "I'm not convinced" in the same cheerful monotone as "I'll take three" is not a customer, it's a text box with a voice.

The honest goal underneath that: I wanted to find out whether emotion in these architectures is a thing you can bolt on after the fact. The published work always adds emotion during training, with the full dataset and the full compute budget. Nobody writes up what happens when you try to add it to frozen released weights, because it mostly doesn't work — and "mostly doesn't work" is a claim I wanted to be able to make precisely rather than assume.

What this is, concretely: four independent experiments across five repositories, each attacking emotion control at a different layer of the stack, plus one shipped iOS application that ended up solving the problem from the opposite direction. Three of the four experiments produced negative results. The fourth is unfinished. The reasons they failed turned out to be more useful than a win would have been, because they're all the same reason wearing different clothes.
...
- **Ran a full adversarial disentanglement experiment and got a clean, causal negative result.** Reconstruction loss rose from 1.13 to 1.29 in lockstep with the confusion weight ramp (0.04 → 0.30), then flatlined the moment the weight stopped rising. That's not noise — it's the model paying a measurable price to comply with an objective it structurally cannot satisfy.
...
- **Measured a voice-embedding space that turned out to be near-degenerate for retrieval.** Across 54 Kokoro built-in voices, the mean pairwise cosine similarity of Kanade global embeddings is **0.9704, with a floor of 0.9023**. A retrieval hit at 0.9869 similarity sounds impressive until you know the worst possible match in the entire library scores 0.90.
- **Quantified cross-space style transfer failure with signal measurements, not adjectives.** Steering a Kokoro voice pack with a StyleTTS2 delta drops output RMS from 0.084 to 0.009 (a 9.4× collapse) as alpha goes 0 → 1.5, while peak amplitude simultaneously hits full-scale clipping at 1.0. The output isn't emotional; it's broken in two directions at once.
...
- **Frozen released weights, not a from-scratch model.** Every experiment had to work against checkpoints someone else trained, with whatever components they chose to publish. This is the constraint that generated all four failures, and it was deliberate — a from-scratch emotional TTS is a compute problem, not an interesting one.
...
- **Small, balanced data over more data.** 4,438 segments, near-perfectly balanced (1,405 default / 1,466 happy / 1,567 sad; 1,527 / 1,472 / 1,439 across the three speakers). I explicitly ruled out data quantity as a cause of failure before blaming the architecture, which mattered because the architecture turned out to be the answer.
...
Recognising this reframed the earlier confusing result. The vectors *did* differentiate — the trained `emotion_table` rows sit 37.951, 38.118, and 38.182 apart from each other — but the audio was overpowering and uncorrelated with the intended emotion. I had read that as a training problem. It was a coordinate problem. The distances being nearly identical across all three pairs is itself the tell: genuinely learned emotion structure would put `happy` and `sad` further from each other than either is from `default`, and instead all three are equidistant to within 0.6%, which is what you get from vectors that have moved but not organised.

The correct decision is the one the original dev plan already had: emotion belongs at the LLM input, the one space where the signal is injected and consumed in the same coordinate system. I got there by elimination rather than by reasoning, which is slower but does produce a result you can defend.
...
Two preprocessing decisions are worth naming because they were not defaults. **Loudness is not normalized** — level and energy are emotion cues, and normalizing them away would have quietly deleted part of the signal I was trying to capture. And the trailing chunk of each utterance is zero-padded rather than dropped, unless it contains less than 8 frames of real audio, in which case it goes. Silence is a harmless reconstruction target; a mostly-silent segment labelled `happy` is not.

I also sidestepped a toolchain problem worth mentioning because it cost real time: `datasets` now wants `torchcodec` to decode audio columns, and `torchcodec` on macOS is an FFmpeg version fight. Casting the column with `Audio(decode=False)` and decoding with `soundfile` manually removes the dependency entirely.

### The encoder, and the two-step training it enabled

The missing tokenizer forced the whole structure. Step 1 trains `SopranoEncoder` to invert the frozen decoder — reconstruction only, neutral emotion id, decoder parameters frozen, multi-resolution STFT loss. There's an alignment subtlety that took a while to get right: the decoder emits `TOKEN_SIZE * (T-1)` samples for a T-frame input, dropping the first frame at the edge, so the reconstruction target is the input audio minus its first 2,048 samples.
...
Every script carries a `--overfit` self-test that validates wiring against decoder-generated targets with no dataset and no checkpoints. The disentanglement test asserts two things separately, which matters: (A) with the adversarial term off, reconstruction loss drops by at least 30%; (B) with it on for one step, all three components — encoder, `emotion_table`, classifier — receive nonzero gradient, and the confusion loss respects its `ln(K)` floor. Conflating these would hide a broken adversary behind working reconstruction, because the two objectives are *supposed* to fight.
...
It's a clean design. The measurement that killed it is in the results below.

## How I kept the evaluation honest

The single most useful methodological choice was writing the decision gates down *before* running anything, in a form that could return "stop." The H1 test plan has a table that says, in as many words, "Step 1 PCA: emotions don't cluster → StyleTTS2 encoder not useful for emotion — stop." The Soprano dev plan gates Day 2 on Day 1 succeeding and says explicitly: "Do not proceed to the steering layer on a broken baseline." Both documents also define what failure looks like in concrete audible terms — "output sounds identical regardless of emotion reference," "emotion transfers but destroys voice identity" — rather than leaving it to post-hoc judgment.

This matters because every one of these experiments had a tempting escape hatch. When the disentanglement run flatlined, the available excuses were data quantity, learning rate, the confusion weight, and a known plumbing bug in `cond_proj` persistence. I worked through them: the plumbing bug was real and I fixed it, and the run still couldn't improve. ~1,100 clips per emotion is not a data-quantity problem when you are fitting **three vectors**. Ruling those out explicitly is what upgraded "it didn't work" into "it can't work, here's why."

The second choice was insisting on blind listening as the primary signal, with two people, on the same 5 sentences × 3 emotions at a fixed speaker. Objective metrics were planned as support — a speech-emotion classifier for recovery of the intended label, Whisper WER for intelligibility at high alpha, speaker-embedding cosine similarity to check emotion wasn't leaking into identity. I should be straight that **those objective metrics were never run**, because the listening gate failed first and the plan said stop. That's the gate working as designed, but it does mean the negative results rest on measured training dynamics plus ears, not on a classifier confusion matrix.

The third: I kept the failure analysis separate from the training guide. `EMOTION_TRAINING.md` documents the route as a how-to and states the expected ceiling up front — "if emotions aren't clearly audible after step 2, the signal is to move conditioning to the LLM, not to grind on the decoder." `EMOTION_FINDINGS.md` records what actually happened. Writing the expectation before the result is what makes the result readable as evidence rather than as rationalization.

## Results

**Every route to model-native emotion control failed. The one system that shipped does not generate emotion at all — it recognises it, and lets a language model respond to it.** That's the honest headline, and the shape of the failures is the interesting part.

### Decoder-side disentanglement: a clean causal negative
...
Meanwhile `cls` and `conf` both pin at 1.098, and `ln(3) ≈ 1.0986`. The adversary is *completely* defeated: the encoder became emotion-invariant almost immediately and the classifier can do no better than uniform guessing. So the disentanglement half of the experiment succeeded perfectly. The encoder gave up its emotion information exactly as asked — and reconstruction got worse rather than the emotion table picking up the slack, which is precisely the signature you would predict if the emotion table lacks the capacity to carry what the encoder dropped.
...
Distinct, and useless. All three pairwise distances agree to within 0.6%, which is the geometry of vectors that have been pushed apart without being organised. Learned emotional structure would not be equilateral.
...
Two failures at once. Signal energy collapses by **9.4×** from alpha 0 to 1.5 — the speech is being progressively erased. And at alpha 1.5 the peak hits exactly 1.0000, full-scale clipping, while RMS is at its lowest. Quiet, mangled output with clipped transients is not an emotion; it's a decoder being fed vectors far outside the distribution it was trained on. Duration also drifts from 1.02 s to as much as 1.82 s on identical text, meaning the prosody predictor is being destabilized rather than steered.

The naive-projection control is worse: broadcasting an averaged StyleTTS2 vector directly as a pack gives `step2_naive_neutral.wav` an RMS of 0.48665 with a peak of exactly 1.0000 — clipped solid — and the angry version 0.04098. An 11.9× energy difference between two supposedly comparable conditions confirms the decoder has no meaningful response to vectors from this space; it just breaks differently depending on the magnitude you hand it.

One correction worth recording, because it's a case of the code being right and my own design doc being wrong: the H1 plan assumed StyleTTS2 emits a `(128,)` vector needing tiling to reach 256 channels. It actually returns `(1, 256)` — the concatenation of `style_encoder` and `predictor_encoder`, 128 each — so the channel dimension already matched and no tiling was needed. The implementation notes this in its docstring and corrects it. The dimensional-expansion risk I had flagged as the primary threat was not the thing that killed the route.

### The retrieval library: a near-degenerate embedding space

This is the measurement I'm most glad I ran, because the system *appeared* to work. A live query returned "Closest voice match: `zm_yunjian` with similarity score: 0.9869," which reads like a confident retrieval.
...
It reached **1,500 of a target 5,000 clips**, and the run at that size overfits: validation audio loss starts diverging at roughly step 200–300, with a best value of ~5.94 around step 100 against a target band of 3–5. The diagnosis is dataset size, and the fix is written down — `SOURCE_HOURS = 10.0`, generate the remaining ~3,500 clips, re-run. I did not do it. This is the one route I abandoned for reasons of time rather than evidence, and I'd call it genuinely undecided rather than failed.

Two operational notes from this line that cost real debugging: MPS fp32 training is stable, but fp16 casting without a `GradScaler` produces NaN within ~100 steps. And the LoRA adapter should not be merged until ship time — merging costs you the ability to swap voices with `set_adapter()`.
...
The design decision I'd defend hardest is that **the score is computed in code, never by the language model.** `compute_score` is deterministic: interest × 5.0 (up to 500 points), objection-resolution ratio × 250, a flat 250 for a win, minus 60 per logged lie, clamped to 1,000, then mapped through fixed grade bands. The judge LLM produces only the qualitative parts — honesty title, flagged claims, coaching tips. It never emits the number. That makes grades reproducible and unit-testable, which a 7B model deciding "this feels like a B+" would not be.

The emotion integration is deliberately partial, and the reasoning is recorded in the source. Since 2026-07-06 the gated SER label drives a composure gate: an angry or sad turn resolves no objections, cannot close the sale, and takes a deterministic meter penalty (8 patience for angry, 5 interest for sad). But the label stays out of `scoring.py` and out of the judge's honesty verdict entirely, because SER accuracy on accented speech is unvalidated. The penalty constants are explicitly flagged as the tuning knob if the gate misfires in playtests. An uncalibrated classifier is allowed to shape the conversation; it is not allowed to decide the grade.

So the project's actual answer to "how do you get emotional speech from a small model" turned out to be: you don't, yet — you build a system that *reads* emotion and responds to it, and let the response carry the meaning the voice can't.

## What didn't work

**The constant-bias emotion table hit a capacity ceiling I had documented and then under-weighted.** `emotion_table` is `nn.Embedding(3, 768)` — three vectors, added time-invariantly to every frame. Real emotion is time-varying. The best this route could ever produce was a timbral shift. **How I found out:** the rising reconstruction loss under the confusion ramp is the model literally paying the cost of an objective it can't satisfy — the encoder gave up emotion information, and the table had nowhere to put it. I wrote the ceiling into `EMOTION_TRAINING.md` before training and still spent the cycles, which is the part I'd flag as a genuine process failure rather than a research one.

**The encoder↔LLM domain mismatch invalidated the route independently of the ceiling.** `emotion_table` was trained on stand-in encoder features but consumed at inference on LLM hidden states — two different distributions with no learned bridge between them. Step 1 makes `encoder ≈ decoder⁻¹`; it never makes `encoder ≈ LLM`. **How I found out:** by trying to explain why the vectors were provably distinct (‖default−happy‖ = 37.951) while the audio was both overpowering and uncorrelated with the intent. No training fix addresses this, because it isn't a training problem — the emotion vector is calibrated against a coordinate system it will never encounter at inference.

**The `cond_proj` train/inference persistence bug — a real bug that was not the cause.** The projection that injects the emotion embedding stayed at random init and wasn't being persisted consistently between training and inference. I found it, fixed it, saved it into the checkpoint alongside the emotion table, and the run still couldn't improve. Worth naming because it's the classic trap: a real bug that is not *the* bug, which will absorb unlimited debugging time if you let it stand in for the architectural problem. There's a residual smell here too — the verification notebook loads `emotion_table` but not `cond_proj`, and two `decoder_emotion.pth` files differ in size (51.0 MB vs 48.7 MB) depending on whether `cond_proj` is included.

**Cross-space style transfer destroyed the signal instead of steering it.** Projecting StyleTTS2 style deltas onto Kokoro packs collapsed RMS 9.4× while driving peaks into clipping, and destabilized duration on fixed text. **Root cause:** StyleTTS2 and Kokoro were fine-tuned separately, so even with matching 256-dim shapes they don't share latent geometry — and Kokoro's conditioning is per-token-position while StyleTTS2's is a single global vector. Matching dimensions is not the same as matching spaces, and the shape compatibility was a false comfort. My design doc had guessed the dimensional expansion would be the killer; the actual killer was geometric.

**Retrieval over Kanade global embeddings was arbitrary, and the similarity scores hid it.** A 0.9869 top-1 match looks decisive until you compute the population statistics and find a mean of 0.9704 and a floor of 0.9023 across 54 voices. **How I found out:** by not trusting a single impressive-looking number and computing the full 54×54 matrix. This is the failure I'd have shipped if I hadn't checked, because everything about it looked like it was working.

**Distillation stalled at 30% of the planned dataset.** 1,500 of 5,000 clips, with validation audio loss overfitting from step 200–300 and a best of ~5.94 against a 3–5 target. **Root cause:** dataset too small — diagnosed, with the remedy written down and not executed. This one is honestly incomplete rather than failed, and the write-up would be stronger if I had finished it.
...
**Verify the injection point before building anything on top of it.** The half-day question I never answered up front — does the decoder consume the LLM's final hidden states or the input code-embedding table? — determines whether the entire decoder-side route is even coherent, and it gates the difficulty of reproducing the tokenizer by inversion. I spent weeks on a route whose viability turned on a question I could have settled in an afternoon with a debugger. This is the single highest-leverage change and it costs almost nothing.

**Send the email on day one.** The whole project was blocked on Soprano's unreleased audio→token encoder. Asking the model's author for it is free, takes minutes, and could have collapsed a multi-week timeline into days. I routed around the missing encoder with an increasingly elaborate stand-in instead of spending five minutes on the path that might have removed the obstacle entirely. Ask for the thing before you build the workaround for not having it.

**Compute the population statistics before trusting any similarity score.** The Kanade retrieval failure was invisible at the level of individual queries and obvious the moment I looked at the distribution. Any system whose behaviour depends on a similarity threshold should report, on day one, what the similarity distribution over the whole reference set actually looks like — mean, floor, percentiles. A single high score means nothing without knowing the range it came from. I'd make this a standing check, not a debugging step.

**Build the evaluation harness first, and share it across experiments.** One script: given generated WAVs and intended labels, report speech-emotion-classifier recovery, Whisper WER, and speaker-embedding cosine similarity against a neutral reference. Every route would then produce comparable numbers, the planned objective metrics would have actually been run, and "this route lost to that one" would be a measurement instead of an impression. The reason my negative results rest on training curves and ears is that I never made running the real metrics cheap.

**Ship the DSP style knob as v1 and keep researching behind it.** The findings doc identifies a 1–2 day, zero-risk fallback: post-process the TTS output — pitch up, faster, brighter for happy; pitch down, slower, darker for sad. It's "acted" rather than model-native, and for a toggle it is frequently indistinguishable to listeners. Having a shipping style mode from week one would have removed the pressure that made each research route feel like it had to succeed, and would have given me a real baseline to measure the model-native routes against. Instead I have four negative results and no controllable emotion in the product. A working cheap thing is not a distraction from the interesting problem — it's the control condition.
